...
// @office-kit/docx (and only those — no ghost names left behind from
...
// it imports the built `dist/index.mjs` artefact.
//
...
const CORE_DIST = resolve(ROOT, "dist/index.mjs");
...
// `previewToDOM` lives in `@office-kit/docx-preview`, not `@office-kit/docx`. The
...
if (stale.length) {
  failed = true;
  console.error(
    `[check:api-page] ${stale.length} name(s) in api-groups.ts are no longer exported from @office-kit/docx:\n` +
      stale.map((n) => `  - ${n}`).join("\n") +
